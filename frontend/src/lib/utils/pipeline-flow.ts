...
export type StepKind = 'generate' | 'analyze' | 'report' | 'wait' | 'notify' | 'script';
...
export function pipelineToFlow(config: PipelineConfig | undefined | null): {
	...
	const allMissingPositions = nodes.length > 0 && nodes.every((n) => {
...
export function flowToPipeline(nodes: WorkflowNode[], edges: Edge[]): PipelineConfig {
	...
	const steps: PipelineStepDSL[] = nodes.map((node, i) => {
		...
		cfg._position = { x: Math.round(node.position.x), y: Math.round(node.position.y) };
		return {
