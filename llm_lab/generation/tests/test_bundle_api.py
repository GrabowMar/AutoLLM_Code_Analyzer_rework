...
from llm_lab.generation.tests.factories import PromptTemplateFactory
...
def test_list_app_templates_is_scoped_to_defaults_and_owner(client):
    ...
    AppRequirementTemplateFactory(is_default=False, created_by=UserFactory())
...
@pytest.mark.django_db
def test_manual_bundle_creation_is_disabled(client):
    user = UserFactory()
    client.force_login(user)
...
def test_list_and_import_starter_package(client):
    ...
    assert import_response.status_code == 200
    payload = import_response.json()
    assert payload["scaffolding_templates"] == ["fastapi-vue", "fastapi-react"]
    assert "analytics_campaign_monitor" in payload["app_templates"]
    assert "system-fastapi-react-standard" in payload["bundles"]
